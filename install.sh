...
export_packages() {
    ...
    local arch_packages=(
        ...
        "base-devel xdotool"
        "libnotify"
        "rofi dunst feh lxappearance connman connman-gtk nitrogen obsession"
        "thunar thunar-archive-plugin thunar-volman lxrandr"
        "pavucontrol pulseaudio mousepad geany geany-common"
        "xdg-user-dirs-gtk lua-language-server"
        "terminology bleachbit gparted synaptic pkexec lxpolkit polkitd"
...
PACKAGES_CORE=(
    ...
    xorg xorg-dev xbacklight xbindkeys xinput
    build-essential xdotool
    libnotify-bin libnotify-dev
)

PACKAGES_UI=(
    rofi dunst feh lxappearance lxpolkit
)
...
PACKAGES_UTILITIES=(
    synaptic gparted nitrogen
    xdg-user-dirs-gtk lua-check
)
